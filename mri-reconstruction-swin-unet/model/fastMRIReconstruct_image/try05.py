...
unsample_kspace = img2kspace(torch.abs(torch.view_as_complex(torch.from_numpy(im[np.newaxis,:,:,:]))),False)
print(np.sum(unsample_kspace.numpy()[0]-kspace))
# masked_kspace_n = masked_kspace[0]
masked_kspace_cplx_n = kspace[:,:,0]+1j*kspace[:,:,1]
#
plt.figure()
...
# plt.show()
# plt.figure()
# plt.imshow(torch.abs(torch.view_as_complex(torch.from_numpy(im))),cmap="gray")
# plt.figure()
# plt.imshow(torch.abs(torch.view_as_complex(unsampled_im[0])),cmap="gray")
plt.figure()
plt.imshow(torch.log(torch.abs(torch.view_as_complex(unsample_kspace[0])) + 1e-9),cmap="gray")
plt.show()