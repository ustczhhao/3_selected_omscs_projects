...
class MRNetDataset(Dataset):
  def __init__(self, args,eval=False):
      ...
      if eval:
          self.dataset_path = args.val_dataset_path
  ...
  def __getitem__(self, index):
    ...
    # undersampled_im = kspace2img(torch.from_numpy(masked_kspace[np.newaxis,:,:,:]),False)
    masked_kspace = masked_kspace.transpose((2,0,1))

    return {'masked_kspace': torch.from_numpy(masked_kspace).to(torch.float32),
            "kspace_gt":torch.from_numpy(kspace_gt).to(torch.float32),
            "im_gt":torch.from_numpy(im_gt).to(torch.float32)
            }